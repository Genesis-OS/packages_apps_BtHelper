...
/**
 * This is the class that does most of the work. It has 3 functions: - Detect when AirPods are
 * detected - Receive beacons from AirPods and decode them (easier said than done thanks to google's
 * autism)
 */
public class PodsService extends Service {
   ...
   *       value is changed (indicated by AT+BIEV command)
   ...
   * Enhanced Safety - 1, Valid Values: 0 - Disabled, 1 - Enabled - Battery Level - 2, Valid Values:
   * 0~100 - Remaining level of Battery
   */
   ...
   * assigned number of the headset indicator as defined by Bluetooth SIG that is being sent. Value
   * range is 0-65535 as defined in HFP 1.7
   */
   ...
   *
   * <p>Always contains the extra fields {@link BluetoothDevice#EXTRA_DEVICE} and
   * {@link BluetoothDevice#EXTRA_BATTERY_LEVEL}.
   */
  ...
  /**
   * Intent used to broadcast bluetooth data update for the Settings Intelligence package's battery
   * widget
   */
   ...
   * things: - Check that the beacon comes from something that looks like a pair of AirPods - Make
   * sure that it is YOUR pair of AirPods - Decode the beacon to get the status
   *
   * <p>After decoding a beacon, the status is written to PodsStatus.
   */
  ...
  private void updatePodsStatus(PodsStatus status, BluetoothDevice device) {
    ...
    if (!isMetaDataSet) {
      isSliceSet =
          setMetadata(
              device, device.METADATA_COMPANION_APP, Constants.AUTHORITY_BTHELPER.getBytes());
      isSliceSet =
          setMetadata(device, device.METADATA_SOFTWARE_VERSION, COMPANION_TYPE_NONE.getBytes());
      isSliceSet =
          setMetadata(
              device,
    ...
    if (!single) {
      ...
      if (!isMetaDataSet) {
        isModelDataSet =
            setMetadata(
                ...
                && setMetadata(
                    device, device.METADATA_MODEL_NAME, regularPods.getModel().getBytes())
                && setMetadata(
                    device,
      ...
      if (statusChanged) {
        ...
        device.setMetadata(
            device.METADATA_UNTETHERED_LEFT_CHARGING, (leftCharging + "").toUpperCase().getBytes());
        device.setMetadata(
            device.METADATA_UNTETHERED_RIGHT_CHARGING,
            (rightCharging + "").toUpperCase().getBytes());
        device.setMetadata(
            device.METADATA_UNTETHERED_CASE_CHARGING, (caseCharging + "").toUpperCase().getBytes());
